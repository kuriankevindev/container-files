...
then
    ...
    docker container run --rm -d --name employee-app -p 8080:8080 -e DB_HOST=employee-db --network employee-net quay.io/kuriankevindev/employee-spring:1.0.0
else
    docker container run -p 8080:8080 quay.io/kuriankevindev/employee-spring:1.0.0
fi
